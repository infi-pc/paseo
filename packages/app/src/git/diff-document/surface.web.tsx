...
// FORK(code-language): only working-tree diffs carry a language scope for code intelligence.
function languageScopeFor(mode: DiffSurfaceProps["mode"]) {
  return mode.kind === "working" ? (mode.languageScope ?? null) : null;
}

export function DiffSurface(props: DiffSurfaceProps) {
  const { t } = useTranslation();
  const toast = useToast();
  const language = useLanguageActions(languageScopeFor(props.mode));
  useEffect(() => {
    language?.dismiss();
