...
function equalFile(left: ParsedDiffFile, right: ParsedDiffFile): boolean {
  ...
  if (
    ...
    return false;
  if (!equalBreakdown(left.breakdown, right.breakdown)) return false;
...
// FORK(change-stats): the categorized breakdown travels with the file; a breakdown-only change
// must still replace the file or the diff tree keeps showing stale category counts.
function equalBreakdown(
  left: ParsedDiffFile["breakdown"],
  right: ParsedDiffFile["breakdown"],
): boolean {
  return left === right || replaceEqualDeep(left, right) === left;
}
...
function equalLines(
  ...
  for (let l = 0; l < left.length; l++) {
    ...
    if (
      x.type !== y.type ||
      x.content !== y.content ||
      x.sourceHunkIndex !== y.sourceHunkIndex ||
      x.sourceLineIndex !== y.sourceLineIndex
    )
      return false;
