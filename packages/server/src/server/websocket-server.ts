...
// FORK(response-control): a finished turn prefers the agent's own completion summary over the
// raw last assistant message for the attention notification.
function attentionAssistantMessage(
  agent: NonNullable<ReturnType<AgentManager["getAgent"]>>,
  reason: "finished" | "error" | "permission",
  fallback: string | null,
): string | null {
  if (reason !== "finished") return fallback;
  return agent.responseMetadata?.lastTurn?.message ?? fallback;
}
...
export class VoiceAssistantWebSocketServer {
  ...
  private async broadcastAgentAttention(params: {
    ...
    const notification = buildAgentAttentionNotificationPayload({
      ...
      assistantMessage: attentionAssistantMessage(agent, params.reason, assistantMessage),
  ...
  // FORK(package-scripts): a finished script terminal reports the script outcome instead of
  // the generic terminal title.
  private terminalAttentionCopy(
    params: { terminalId: string; terminalName: string; reason: TerminalAttentionReason },
  ...
  private async broadcastTerminalAttention(params: {
    ...
    const { title, body } = this.terminalAttentionCopy(params, workspaceId);
