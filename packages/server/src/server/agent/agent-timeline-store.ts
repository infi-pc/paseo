...
export class InMemoryAgentTimelineStore {
  ...
  // FORK(tool-call-summaries): summary targets remember the seq of the source event. The
  // projection collapses a tool call's lifecycle into one row keyed by its last seq, so resolve
  // the target through the row's source ranges rather than its seq.
  getRow(agentId: string, seq: number): AgentTimelineRow | null {
    const row = this.requireState(agentId)
      .projection.getRows()
      .find((entry) =>
        entry.sourceSeqRanges.some((range) => range.startSeq <= seq && seq <= range.endSeq),
      );
